...
abstract contract BaseStrategy {
    ...
    /**
     * @notice
     *  计算利润和亏损、调整仓位,通常被Vault.report()方法调用,
    ...
    function harvest() external onlyKeepers {
        uint256 profit = 0;
        uint256 loss = 0;
        uint256 debtOutstanding = vault.debtOutstanding();  //可以从策略上赎回的债务 : 实际债务 - 债务上限
...
contract Strategy is BaseStrategy {
    ...
    ICurveFi public constant StableSwapSTETH =  ICurveFi(0xDC24316b9AE028F1497c275EB9192a3Ea0f67022);  // curve steth Pool,  ETH <---> stETH
    ...
    // We hard code a peg[挂钩] here. This is so that we can build up a reserve of profit[收益储备池] to cover peg volatility if we are forced to delever[杠杠]
    // This may sound scary but it is the equivalent of using virtualprice in a curve lp. As we have seen from many exploits, virtual pricing is safer than touch pricing.[虚拟价格  接触价格]
    //stETH * 99%  +  在储蓄罐的余额
    function estimatedTotalAssets() public override view returns (uint256) {
        // DENOMINATOR 10000(100%), peg: 100(1%)
    ...
    /**
     * _debtOutstanding为 （实际负债-负债上限） 计算出来
     * 待提现的本币(WETH)为： 收益(利润)+调出债务
     * 返回：待提现的本币
     */
    function prepareReturn(uint256 _debtOutstanding)
        internal
    ...
    {
        uint256 wantBal = wantBalance();  //want 就是vault,当前策略在储蓄罐里持有yWETH份额
        uint256 totalAssets = estimatedTotalAssets();  // 当前储蓄罐的资产

        uint256 debt = vault.strategies(address(this)).totalDebt; //当前储蓄罐负债
