...
# https://yearn.watch/vault/0xa258C4606Ca8206D8aA700cE2143D7db854D168c
...
@notice
    ...
    impact. (Funds are taken from the Strategy that will disturb(影响) everyone's
    ...
    【紧急暂停模式，资产尽快从策略上撤回，实现最小化损失，不允许存入、可以提现】
...
totalSupply: public(uint256)   # 总份额

token: public(ERC20)  #原始ERC20 Token地址，该合约对应的是WETH
...
event StrategyAdded:
    ...
    debtRatio: uint256  # Maximum borrow amount (in BPS of total assets)  是不是可以理解为，该策略可以投资占比
    minDebtPerHarvest: uint256  # Lower limit on the increase of debt since last harvest  
    maxDebtPerHarvest: uint256  # Upper limit on the increase of debt since last harvest
    performanceFee: uint256  # Strategist's fee (basis points)   收益占比


event StrategyReported:
    strategy: indexed(address)
    gain: uint256  # 收益 
    loss: uint256  # 损失 和上面的gain有啥区别
    debtPaid: uint256  # 债务支付
    totalGain: uint256  # 总收益
    totalLoss: uint256  
    totalDebt: uint256
    debtAdded: uint256
    debtRatio: uint256
...
def initialize(
    ...
    self.performanceFee = 1000  # 10% of yield (per Strategy)  10%的收益分成
...
# 合约授权,授权spender，可以从我的地址转amount个token到当前合约
@external
...
# owner 签名批准spender花费owner的token额度
@external
...
# 总资产= 当前Vault合约的WETH余额+总负债,totalDebt为投资到策略里的资金
@view
@internal
...
def totalAssets() -> uint256:
    ...
    @notice
        ...
        the Vault.
        返回当前储蓄罐的控制的总资产，包括借出给策略投资和当前持有资资产 
...
def _calculateLockedProfit() -> uint256:
    # 锁定按时间线性释放
    # (当前区块时间 - 上次报告时间) *  lockedProfitDegradation 每个区块时间的锁定释放比例
...
# 被deposit()方法调用  发行存入代币梳理对应的yToken份额
@internal
def _issueSharesForAmount(to: address, amount: uint256) -> uint256:
    ...
    //balance里加了增加了token，需要生成一个事件，记录从零地址转入到to
...
def deposit(_amount: uint256 = MAX_UINT256, recipient: address = msg.sender) -> uint256:
    ...
    @notice
        ...
        call will fail.
    @dev
        计算新的发行的份额数量,基于总的outstanding债务
        expected value 和  estimated value 啥区别  ??
        安全考虑因素是啥？？  
        ...
        获取的份额有可能低于存入的份额，存入n个WETH，得到的yWETH可能小于n
        不然，如果大于，把套利者攻击，然后榨干池子
    ...
    # up to deposit limit
    #  如果depositLimit规模是100000，当前是9500，那我存6000个，最终只能存5000，不能超过depositLimit
...
# 最大可用(可赎回)份额,将每个策略地址的总负债对应的份额相加
@view
@external
...
def withdraw(
    ...
    @notice
        ...
        amount `_shares` for an appropriate[合适 适当] amount of tokens.

        See note on `setWithdrawalQueue` for further details of withdrawal
        ordering and behavior.

        实际赎回资产数不一定是请求赎回份额数，因为策略不一定够  
    @dev
        Measuring the value of shares is based on the total outstanding debt【未偿还债务】
        that this contract has ("expected value") instead of the total balance
        sheet it has ("estimated value") has important security considerations, [不是根据各项投资的估值决定，而是基于一个expected value]
        and is done intentionally. If this value were measured against external [如果根据外部实时估值，黑客容易超控价格，形成套利]
        ...
        withdrawing shares via this method could entitle【给予】 the depositor to
        *more* than the expected value once the "realized value" is updated
        ...
        【如果通过外部系统衡量估值，在一些异常场景下，早期发起赎回者，能赎回更多资产】
        ...
        【当大量赎回发生，储蓄罐的余额和赎回策略队列会被挤兑】
        不在赎回队列里的策略将会执行调仓，然资产再策略上分布再平衡
    @param maxShares
    ...
    # See @dev note, above.
    #计算份额(yWETH)对应的资产数量(WETH)
    value: uint256 = self._shareValue(shares)

    totalLoss: uint256 = 0
    # 提取的资产大于储蓄罐可用金额，则需要从策略里赎回一些资产，用来提现
    if value > self.token.balanceOf(self):
        ...
        for strategy in self.withdrawalQueue:
            ...
            #strategy.withdraw()直接将WETH转入到Vault合约地址，当前WETH余额减去vault_balance,就是赎回到账WETH数量
            ...
            # 投资人的存款就是vault的负债Debt 
            self.totalDebt -= withdrawn

        # NOTE: We have withdrawn everything possible out of the withdrawal queue
        #       but we still don't have enough to fully pay them back, so adjust
        #       to the total amount we've freed up through forced withdrawals
        # 如果上面赎回的vault_balance小于用户份额对应的WETH(value)，则仅赎回vault_balance
        vault_balance: uint256 = self.token.balanceOf(self)
        if value > vault_balance:
            value = vault_balance
...
# 更新债务比率，debtRatio不是绝对数字，而是相对调整数字
@external
...
def updateStrategyMinDebtPerHarvest(
    ...
    @notice
        ...
        更新每个策略的债务比例之后，可能需要存入资产或赎回资产 在该策略上，保持债务占比和实际该策略资产平衡，但是资产一直变化，实时调仓，执行有成本，需要设置一个最小的偏离值
...
@view
@external
#根据债务比例，计算该策略待调整债务，计算公式： 场景1( 实际债务 > 债务上限 ):  实际债务 - 债务上限
...
# 判断一个策略还有多少额度可以投资，计算公式： min( (该策略负债上限 - 该策略当前实际负债), (储蓄罐负债上限 - 储蓄罐当前实际负债) )
@view
@external
def creditAvailable(strategy: address = msg.sender) -> uint256:
    ...
    @return The quantity of tokens available for the Strategy to draw on.  draw on  是凭借、利用的意思
    """
    return self._creditAvailable(strategy)


# 期望市值根据上次报告市值 + 上次报告以来随时间线性增值部分
# 那这样会有什么问题？？  深入思考下
@view
@internal
...
# 返回策略的期望值市值
# 用期望市值而不是实时市值来评估用户可赎回份额，防止黑空操控和攻击
@view
@external
...
def report(gain: uint256, loss: uint256, _debtPayment: uint256) -> uint256:
    ...
    @param gain
        Amount Strategy has realized as a gain【获利】 on it's investment since its
        last report, and is free to be given back to Vault as earnings
    @param loss
        Amount Strategy has realized as a loss 【亏损】 on it's investment since its
    ...
    debt: uint256 = self._debtOutstanding(msg.sender)  # 待偿还债务
